...
import {
  ...
  type Matcher,
} from '@/components/molecules/common/Calendar';
  ...
  return (
    <div
      ...
      className={cx(
        ...
        focusInput,
        // invalid (optional)
        'group-aria-[invalid=true]/time-input:dark:ring-red-400/20 invalid:border-red-500 invalid:ring-2 invalid:ring-red-200 group-aria-[invalid=true]/time-input:border-red-500 group-aria-[invalid=true]/time-input:ring-2 group-aria-[invalid=true]/time-input:ring-red-200',
        {
          '!w-fit border-none bg-transparent px-0 text-gray-400 shadow-none': isDecorator,
          hidden: isSpace,
          'border-border-muted bg-background-light text-text-muted': state.isDisabled,
    ...
    return (
      <PopoverPrimitives.Trigger asChild>
        <button
          ...
          {...props}
        >
          <RiCalendar2Fill className="size-5 shrink-0 text-text-muted" />
          <span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left text-text">
            {children ||
              (placeholder ? <span className="text-text-muted">{placeholder}</span> : null)}
          </span>
        </button>
  ...
  return (
    <PopoverPrimitives.Portal>
      <PopoverPrimitives.Content
        ...
        className={cx(
          // base
          'shadow-black/[2.5%] relative z-50 w-fit rounded-md border text-sm shadow-xl',
          // widths
        ...
        return (
          <li key={index} className="sm:w-full sm:py-px">
            <button
              ...
              className={cx(
                // base
                'relative w-full overflow-hidden text-ellipsis whitespace-nowrap rounded border px-2.5 py-1.5 text-left text-base shadow-xs outline-hidden transition-all sm:border-none sm:py-2 sm:text-sm sm:shadow-none',
                // text color
                'text-text',
                // border color
                'border-border',
                ...
                focusRing,
                // background color
                'focus-visible:bg-background-light',
                'hover:bg-background-light',
                {
                  'bg-background-light': matchesCurrent(preset),
                }
              )}
...
interface RangeProps extends PickerProps {
  ...
  value?: DateRange;
  onChange?: (_dateRange: DateRange | undefined) => void;
}
...
function DatePicker({ presets, ...props }: SingleDatePickerProps) {
  if (presets) {
...
function DateRangePicker({ presets, ...props }: RangeDatePickerProps) {
  if (presets) {
    validatePresets(presets, props);
  }
